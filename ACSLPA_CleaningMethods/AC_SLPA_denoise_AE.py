...
def SVM_1(X):
    SVM=svm.OneClassSVM(nu=0.1, kernel="rbf")
...
def Detect_noisy_pairwise_constraints_must(G,constraints,scores,N2V_model,degrees,node_map2,threshold,AE_Model,must_discard_5,AE_Model_var_must):
    ...
    for nodes in constraints:
        ...
        pair.append(nodes[0])
        pair.append(nodes[1])
        pair.append(int(1)) # label for musr_link
    ...
    pairwise_cost_norm=prepare_data_without_label(pairwise_const_features)
    scores, AE_Model_var_must=AE(pairwise_cost_norm,AE_Model,AE_Model_var_must)

    scores = scores.reshape(scores.shape[-1],1)
    pairwise_const_results=np.concatenate((pairwise_cost_pairs,scores), axis=1)
    pairwise_const_results = pairwise_const_results[pairwise_const_results[:,3].argsort()] # lowest to highest
...
def Detect_noisy_pairwise_constraints_cannot(G,constraints,scores,N2V_model,degrees,node_map2,threshold,AE_Model,cannot_discard_5,AE_Model_var_cannot):
    ...
    scores, AE_Model_var_cannot=AE(pairwise_const_norm,AE_Model,AE_Model_var_cannot)
...
def main():
    ...
    node_map, must_constraints_tobenoised, cannot_constraints_tobenoised = noise_const_method(communities_groundtruth,
                                                                                              num_must_noise,
                                                                                              num_must_noise, G)
    print('must_constraints_tobenoised', len(must_constraints_tobenoised), 'cannot_constraints_tobenoised',
          len(cannot_constraints_tobenoised))
    ...
    while len(Total_Constraints) < budget:
        ...
        for i in range(5):
            if i <= 2:
                if len(must_constraints_Total)>=min_chunck_size:
                    must_constraints_Total_1, must_discard, AE_Model_var_must=Detect_noisy_pairwise_constraints_must(G,must_constraints_Total,scores,N2V_model,degrees,node_map2,0.95,AE_Model_must,must_discard,None)
        ...
        if ( len(Total_Constraints) >= budget or len(temp) == len(Total_Constraints) or (len(Constraints) < termination and iteration_copy>1)) and Total_Constraints != 0 :
            if len(Total_Constraints) < min_chunck_size:
                continue
            else:
                break
    ...
    for i in range(1):
        must_constraints_Total_1, must_discard, AE_Model_var_must=Detect_noisy_pairwise_constraints_must(G,must_constraints_Total_cleaned,scores,N2V_model,degrees,node_map2,1,AE_Model_must,must_discard, None)
        cannot_constraints_Total_1, cannot_discard, AE_Model_var_cannot=Detect_noisy_pairwise_constraints_cannot(G,cannot_constraints_Total_cleaned,scores,N2V_model,degrees,node_map2,1,AE_Model_cannot,cannot_discard, None)
    ...
    percentage_cannot=0.85
    percentage_must=0.90
    ...
    while(True):
        ...
        for k in range(7):
            print ('Start Cleaning discarded labels')
            if condition_must==True and len(must_discard)!=0:
                if i <= 4:
                    must_discard_11, must_discard_1, AE_Model_var_must=Detect_noisy_pairwise_constraints_must(G,list(must_discard),scores,N2V_model,degrees,node_map2,percentage_must,AE_Model_must,must_discard_1,AE_Model_var_must)
            ...
            if condition_cannot==True and len(cannot_discard)!=0:            
                cannot_discard_11, cannot_discard_1, AE_Model_var_cannot=Detect_noisy_pairwise_constraints_cannot(G,list(cannot_discard),scores,N2V_model,degrees,node_map2,percentage_cannot,AE_Model_cannot,cannot_discard_1,AE_Model_var_cannot)
            ...
            percentage_cannot=percentage_cannot+0.02
            percentage_must=percentage_must+0.02

        print ('Cleaning discarded labels loop finish')
    ...
    for i in range(1):
        print ('Final_loop')
        must_constraints_Total_1, must_discard, AE_Model_var_must=Detect_noisy_pairwise_constraints_must(G,must_constraints_Total_cleaned,scores,N2V_model,degrees,node_map2,0.99,AE_Model_must,must_discard, AE_Model_var_must)
        cannot_constraints_Total_1, cannot_discard, AE_Model_var_cannot=Detect_noisy_pairwise_constraints_cannot(G,cannot_constraints_Total_cleaned,scores,N2V_model,degrees,node_map2,0.99,AE_Model_cannot,cannot_discard, AE_Model_var_cannot)
